...
function Espacios({ onOpen }: { onOpen: (k: string, n?: string) => void }) {
  ...
  // destructivo → siempre por ConfirmDialog, nunca de un click
  const delEsp = async () => { if (!openId) return; setModal(null); await deleteEspacio(openId).catch(() => {}); setOpenId(null); setDetail(null); loadList() }
      ...
      <div className="pane">
        ...
        {dLoading && !detail ? <div className="center" style={{ height: 160 }}><div className="spin" /></div> : (
          <div className="panebody">
            ...
            <button className="mbtn ghost" style={{ width: "100%", marginTop: 8, color: "var(--danger)" }} onClick={() => setModal({ kind: "del" })}>Eliminar espacio</button>
          </div>
        )}
        ...
        {modal?.kind === "del" && <ConfirmDialog title={<>¿Eliminar el espacio {d.name}?</>} body="No se borra ningún mensaje: solo dejan de agruparse acá. Si tiene subespacios, quedan sueltos." confirmLabel="Eliminar espacio" onCancel={() => setModal(null)} onConfirm={delEsp} />}
      </div>
    )
  }
